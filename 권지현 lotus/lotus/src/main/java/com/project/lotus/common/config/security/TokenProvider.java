...
public class TokenProvider {
    ...
    // Access Token 1시간 설정 *24.01.21 jihyun
    private static final long ACCESS_TOKEN_EXPIRED_TIME = 1000 * 60 * 60;

    // Refresh Token 1주일 설정 *24.01.22 jihyun
    private static final long REFRESH_TOKEN_EXPIRED_TIME = 1000 * 60 * 60 * 7;
    ...
    public String validateRefreshToken(RefreshToken refreshTokenObj){
        ...
        // Refresh Token 객체에서 refreshToken 추출 *24.01.21 jihyun
        String refreshToken = refreshTokenObj.getRefreshToken();

        try {
            // Refresh Token으로 Claims 추출함 *24.01.21 jihyun
            Jws<Claims> claims = Jwts.parser().setSigningKey(secretKey).parseClaimsJws(refreshToken);
            // Refresh Token 만료시간이 지나지 않았을 경우, 새로운 access 토큰을 생성함 *24.01.21 jihyun
    ...
    // Access Token 인증 타입 제거 *24.01.18 jihyun
