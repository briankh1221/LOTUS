  ...
  <div>
    <form>
      <div class="container text-center">
        <h2 class="justify-items-center mb-5">상품 수정</h2>
        <div class="row">
          <div class="col-2"></div>
            ...
            <div class="input-group">
              ...
              <div class="input-group-text p-1">
                <input
                  ...
                  id="transactionStatus1"
                  value="ON_SALE"
                />
                <label
                  class="btn btn-outline-success mx-1"
                  for="transactionStatus1"
                  >판매중</label
                >
                <input
                  v-model="form.transactionStatus"
                  ...
                  class="btn-check"
                  name="transactionStatus"
                  id="transactionStatus2"
                  value="UNDER_RESERVATION"
                />
                <label
                  class="btn btn-outline-secondary mx-1"
                  for="transactionStatus2"
                  >예약중</label
                >
                <input
                  v-model="form.transactionStatus"
                  ...
                  class="btn-check"
                  name="transactionStatus"
                  id="transactionStatus3"
                  value="COMPLETED"
                />
                <label
                  class="btn btn-outline-secondary mx-1"
                  for="transactionStatus3"
                  >거래완료</label
                >
              </div>
            </div>
...
const { productIdx } = history.state;
...
const productModify = async () => {
  try {
    ...
    if (form.value.transactionStatus)
    ...
    form.value.images.forEach(file => {
      if (typeof file == 'string') {
        formData.append('images', new File([], file));
      } else {
    ...
    await formInstance.put('/user/product/' + productIdx, formData);
    alert('상품 수정 완료');
...
onMounted(async () => {
  form.value = await fetchDetail();
  selectedFiles.value = form.value.images.split(' ');
  form.value.images = [...selectedFiles.value];
});
</script>
