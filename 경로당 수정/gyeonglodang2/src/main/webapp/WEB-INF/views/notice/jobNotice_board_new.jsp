...
// 행 수정 감지 Event Listener 설정 *231110 권지현
function update() {
...
function rowDown(obj) {
	...
	// 현재행과 다음행 위치 교환 *231111 고훈
	if (tr2) {
	    $(tr1).insertAfter(tr2); 
	}
 }

// 페이지 진입 상태에 따른 저장, 수정 버튼 설정 *231112 고훈
function buttons() {
...
function increaseNumber(string) {
 	// show, hidden 포함 총 행 개수 확인 *231111 고훈
    const allTrCnt = $('tr.displayRow').length + $('tr.hiddenRow').length;
	// 새로운 행 번호 설정 *231111 고훈
...
window.onload = function() {
	...
	// 채용 공고 셀렉트를 위한 백단 세션에서 noticeControl_idx 가져오는 매서드
	getSession();
}
