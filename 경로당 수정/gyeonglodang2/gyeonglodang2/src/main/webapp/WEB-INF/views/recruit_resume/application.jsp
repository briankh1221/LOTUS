...
<script>
	$(document).ready(function(){
		$('#submit_form').on("click", function () {
			console.log("check");
			$('#form').attr("action", "../recruit_resume/insertApplication");
			$('#form').submit();
		}); 
		$('#final_submit').on("click", function () {
			$('#form').attr("action", "insertFinal");
			$('#form').submit();
		});
	});
...
<body>
	...
	<div id="wrap">
		<div id="content">
			<form name="applicant_form" action="" id="form" enctype="multipart/form-data" method="post">
				<h2>지원서 작성</h2>
				<div class="field">
					<table id="fieldtbl">
						<tr>
				...
				<c:forEach items="${resumecustom_list }" var="dto" varStatus="loop">
					<p id="customTitle">${dto.customTitle }</p>
					<div class="customQuestionnotice">
															...
															<c:if test="${tqDto.tableTypeidx eq tDto.idx }">
																...
																<c:when test="${not empty tableValueList }">
																	...
																	<c:if test="${tableloop.count eq countTemp_table }">
																		<td><input type="text" name="tableText${loop.count }-${temp}"
																			 value="${tableValue }"></td>
																	</c:if>
																	</c:forEach>
				...
				<div id="buttons">
					<input type="button" id="submit_form" value="저장하기">
